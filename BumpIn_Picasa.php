<?php
/*
	Plugin Name: Bumpin Picasa Photos
...
function render_bumpin_picasa( $opt, $before_title, $after_title) {
...
function widget_bumpin_picasa_control( $args=null ) {
...
?>
		<p style="text-align:justify;">It seems you are already using BumpIn SocialBar widgets. </p>
                <p style="text-align:justify; color:red;">If you dont see the "Picasa" app in the SocialBar at your site, Please follow the steps below.</p>
		<span style="text-align:justify;">
                    ...
                    <ol>
                        ...
                        <li>Click on the "Edit Socialbar" link, click the "Picasa" app in the "Select Applications" tab.</li>
                        <li>Hit the "Update" button and refresh your website to start using Picasa in the BumpIn SocialBar.</li>
                   </ol>
		</span>
		<p>
...
?>
	...
	<p style="text-align:justify;">
		...
		To get the code for the bar <a href = "http://socialbar.ticketmy.com/checkout?wordpress=true&plugin=true&picassa=true" target = "_blank">Click Here</a>
	</p>
	<p>
		<textarea style="width: 590px; height: 150px;" id="bumpin_code" name="bumpin_code"><?php echo $bumpin_code; ?></textarea>
		<input type="hidden" id="submit_code" name="submit_code" value="1" />
...
function widget_bumpin_picasa($args) {
	extract($args);
	$options = get_option('BumpIn');
	echo $before_widget;
	render_bumpin_picasa($options,$before_title,$after_title);
  	echo $after_widget;
}

function bumpin_picasa_init()
{
	register_sidebar_widget(__('Picasa Photos'), 'widget_bumpin_picasa');
	register_widget_control('Picasa Photos','widget_bumpin_picasa_control',600,600);    /* width, height*/
}
add_action("plugins_loaded", "bumpin_picasa_init");
?>
